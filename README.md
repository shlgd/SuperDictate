...
```bash
curl -fsSL https://raw.githubusercontent.com/shlgd/SuperDictate/v0.2.39/install.sh | /usr/bin/arch -arm64 /bin/bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/shlgd/SuperDictate/v0.2.39/install.sh | SUPERDICTATE_BUILD_FROM_SOURCE=1 /usr/bin/arch -arm64 /bin/bash
```
...
- `Настройки → Text → Keep History For` — сколько расшифровки живут на диске:
  бессрочно (по умолчанию), 1 день, 7 дней или 30 дней. Просроченные записи
  удаляются при запуске, при записи в историю, при открытии оверлея истории и
  ежечасной проверкой.
- `Настройки → Text → Recent Transcripts` — отдельное ограничение по количеству
  записей в оверлее. Лимиты складываются: запись должна проходить оба. Значение
  `Off` не хранит историю вовсе.
- `Настройки → Text → Clear Private Data Now…` — удаляет все сохранённые
  расшифровки и оставшееся аудио аварийного восстановления, с подтверждением.
  Настройки, пользовательские замены и модель не затрагиваются.

По умолчанию выбрано бессрочное хранение, чтобы обновление не удаляло историю
без явной просьбы. Записи, сохранённые до появления этой настройки, получают
дату при первом запуске новой версии и начинают отсчёт с этого момента.

Аудио аварийного восстановления старше 24 часов удаляется автоматически
независимо от настройки хранения: это буфер на случай падения, а не история.
...
```bash
curl -fsSL https://raw.githubusercontent.com/shlgd/SuperDictate/v0.2.39/uninstall.sh | bash
```
