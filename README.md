...
- Чтобы назначить другую клавишу или сочетание, откройте SuperDictate,
  нажмите `Изменить…`, введите новый вариант и подтвердите его кнопкой
  `Сохранить`. Фоновая служба перезапустится автоматически.
- Откройте `SuperDictate` из Applications, чтобы проверить службу,
  разрешения и настройки.

Панель управления по умолчанию открывается на русском. Переключатель `RU / EN`
в правом верхнем углу мгновенно меняет язык. Во время запуска или перезапуска
фоновой службы панель показывает текущую операцию и остаётся отзывчивой.
...
Откройте панель SuperDictate. В блоке `Обновления` приложение само покажет
новую версию. Нажмите `Обновить до v…`: архив скачается, пройдёт проверку
SHA-256, bundle ID, версии и подписи, после чего приложение заменит себя и
откроется заново. Предыдущая версия автоматически восстанавливается при ошибке.

Команда остаётся запасным способом:

```bash
curl -fsSL https://raw.githubusercontent.com/shlgd/SuperDictate/main/install.sh | bash
```

Ничего не устанавливается без нажатия кнопки обновления.
